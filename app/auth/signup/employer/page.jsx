...
export default function EmployerSignup() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      if (profile?.role) {
        ...
        // new google user — prefill email and wait for continue
        setGoogleUser(user);
        setEmail(user.email);
  ...
  async function handleGoogleSignup() {
    await getSupabase().auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/signup/employer`,
      }
    });
  }
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5 py-16">
        <div className="w-full max-w-sm flex flex-col gap-6">
          <div className="text-center">
            ...
            <p className="text-gray-500 text-sm mt-1">Start finding gigs in your town</p>
          </div>

          {/* if google user is detected, show prefilled state */}
          {googleUser ? (
            <>
              <div className="w-full flex items-center gap-3 border border-gray-200 rounded-xl p-4 bg-gray-50">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900">{googleUser.email}</p>
                  <p className="text-xs text-gray-400">Connected with Google</p>
                </div>
              </div>
